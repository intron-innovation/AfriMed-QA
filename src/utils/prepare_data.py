...
def transform_mcqs(data):
    ...
    for _, row in tqdm(data.iterrows(), total=len(data), desc="Preprocessing the data"):
        ...
        option_keys = [f"option{i+1}" for i in range(5)]  # Adjust number of options if needed
        answer_labels = 'ABCDE'
        
        # Extract and label options
        formatted_options = {label: options.get(key, '') for label, key in zip(answer_labels, option_keys) if key in options}
